...
public class Cadena<E> implements Iterable<E> {
	...
	public boolean eliminar(int idx) {
		if (raiz == null) {
			return false;
		}
		...
		while (idxActual < idx && actual.getSiguiente() != null) {
			idxActual++;
		...
		// La unica opcion es que sea el id correcto a eliminar
		if (cola == actual.getSiguiente())
			cola = actual;
	...
	public String toString() {
		...
		String separador = "";
		while (actual != null) {
			sb.append(separador).append(actual.getValor());
			actual = actual.getSiguiente();
			separador = "->";
		}
		return sb.toString();
